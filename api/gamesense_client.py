...
class GameSenseClient:
    ...
    def register(self):
        """Register widget as a GameSense game."""
        payload = {
            "game": GAMESENSE_GAME,
        ...
        resp = self._session.post(
            f"{self.base_url}/game_metadata", json=payload, timeout=5,
        )
        resp.raise_for_status()
        self.registered = True

    def bind_color_handler(self, device_type: str = "rgb-per-key-zones",
                           zone: str = "all"):
        """Bind a color handler so we can send arbitrary RGB values."""
        payload = {
            "game": GAMESENSE_GAME,
        ...
        resp = self._session.post(
            f"{self.base_url}/bind_game_event", json=payload, timeout=5,
        )
        resp.raise_for_status()

    def send_color(self, red: int, green: int, blue: int) -> bool:
        """Send an RGB color event to devices."""
        payload = {
            "game": GAMESENSE_GAME,
            "event": GAMESENSE_EVENT,
            "data": {
        ...
        try:
            resp = self._session.post(
                f"{self.base_url}/game_event", json=payload, timeout=3,
            )
            return resp.ok
        except Exception:
            return False
    ...
    def cleanup(self):
        """Remove game registration on exit."""
        try:
            self._session.post(
