...
class SonarPresetsClient:
    """Manage Sonar EQ preset configs - list favorites, get selected, switch."""
    ...
    def get_all_configs(self) -> list[dict] | None:
        """Fetch all EQ configs from Sonar."""
        try:
    ...
    def get_configs_by_device(self, favorites_only: bool = True) -> dict[str, list[dict]] | None:
        """Get configs grouped by virtualAudioDevice.

        Args:
            favorites_only: If True, return only favorites. If False, return all.

        Returns:
            {"game": [{"id": ..., "name": ..., "isFavorite": bool}, ...], ...}
        """
    ...
    def get_selected_configs(self) -> dict[str, dict] | None:
        """Get currently selected configs, keyed by device.

        Returns:
            {"game": {"id": ..., "name": ...}, "chatRender": {...}, ...}
        """
        try:
            resp = self._session.get(
                f"{self.base_url}/configs/selected", timeout=5,
            )
            resp.raise_for_status()
            data = resp.json()
            result = {}
    ...
    def select_config(self, config_id: str) -> bool:
        """Activate a config by its ID. PUT /configs/{id}/select"""
        try:
