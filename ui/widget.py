...
class GGWidget:
    """Main widget - modern, borderless, draggable, always-on-top."""
    ...
    def __init__(self, root: ctk.CTk, on_quit_callback=None):
        ...
        # Window configuration — start hidden to prevent taskbar flash
        root.withdraw()
        ...
        # Win32 setup — runs after Tk maps the window
        root.after(50, self._setup_win32)
    ...
    # =================================================================
    # UI Construction (split into methods for rebuild_ui support)
    # =================================================================
    ...
    def _build_title_bar(self):
        ...
        # Close (rightmost) - quits the application
        ctk.CTkButton(
            self.title_bar, text="\u2715", width=32, height=28,
        ...
        ctk.CTkLabel(
            ...
            font=("Segoe UI", 10), text_color=styles.TEXT_MUTED,
    ...
    def _build_menu(self):
        ...
        ctk.CTkLabel(
            self._menu_frame, text="Tema", font=("Segoe UI", 12, "bold"),
    ...
    def set_rebuild_callback(self, callback):
        """Register a callback that recreates panel contents after theme change."""
        self._on_rebuild_cb = callback

    def rebuild_ui(self):
        """Destroy and recreate the entire UI with current theme colors."""
    ...
    def _change_theme(self, name: str):
        """Switch theme and rebuild the entire UI — no restart required."""
    ...
    # =================================================================
    # Win32 — taskbar hiding + rounded corners
    # =================================================================

    def _get_hwnd(self):
        """Return the top-level HWND for the Tk window."""
        frame = self.root.winfo_id()
        hwnd = _user32.GetParent(frame)
        if not hwnd:
            hwnd = _user32.GetAncestor(frame, GA_ROOT)
        return hwnd or frame

    def _setup_win32(self):
        """One-time Win32 setup: hide from taskbar + rounded corners."""
        try:
            hwnd = self._get_hwnd()
            self._hwnd = hwnd
        ...
        # Show the window (it was withdrawn at startup)
        self.root.deiconify()
        self.root.attributes("-topmost", True)

        # Apply rounded corners after the window is visible
        self.root.after(30, self._apply_rounded_corners)

    def _apply_rounded_corners(self):
        """Clip the window to a rounded rectangle region."""
        try:
    ...
    def hide(self):
        """Hide the widget window (tray only)."""
        self.root.withdraw()
    ...
    def _toggle_minimize(self):
        if self._minimized:
            self.content.pack(fill="both", expand=True)
            self.root.geometry(
                f"{self.root.winfo_width()}x{WIDGET_HEIGHT}"
            )
            self._minimized = False
        else:
        ...
        # Update rounded corner mask for new size
        self.root.after(30, self._apply_rounded_corners)
