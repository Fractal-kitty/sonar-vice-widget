...
class RGBPanel:
    """RGB lighting controls - modern UI."""
    ...
    def __init__(self, parent: ctk.CTkFrame, gamesense_client):
        ...
        ctk.CTkLabel(
            rgb_card, text="\U0001F3A8  RGB Lighting", font=FONT_SECTION,
            text_color=styles.TEXT,
        ).pack(anchor="w", padx=16, pady=(12, 8))

        if not self.gs:
            ctk.CTkLabel(
                rgb_card, text="GameSense baglantisi yok",
                font=FONT_SMALL, text_color=styles.RED,
            ).pack(pady=(0, 16))
            return
        ...
        ctk.CTkLabel(
            bottom, text="Preview", font=FONT_SMALL, text_color=styles.TEXT_DIM,
        ).pack(side="left")
        ...
        ctk.CTkButton(
            bottom, text="Apply", width=80, height=28,
            font=FONT_BODY, fg_color=styles.ACCENT, hover_color=styles.ACCENT_HOVER,
            text_color=styles.TEXT, corner_radius=8,
            command=self._send_color,
        ).pack(side="right")
        ...
        for name, r, g, b in presets:
            ctk.CTkButton(
                preset_frame, text=name, width=50, height=26,
                font=FONT_SMALL, fg_color=styles.BG_HOVER, hover_color=styles.ACCENT,
                text_color=styles.TEXT, corner_radius=6,
                command=lambda r=r, g=g, b=b: self._apply_preset(r, g, b),
    ...
    def _create_color_slider(self, parent, label: str, color: str, var: ctk.IntVar):
        ...
        slider = ctk.CTkSlider(
            ...
            fg_color=styles.BG_HOVER, progress_color=color,
        ...
        ctk.CTkLabel(
            row, textvariable=var, font=FONT_SMALL,
            text_color=styles.TEXT_DIM, width=30,
        ).pack(side="right")
    ...
    def _send_color(self):
        ...
        r, g, b = self.r_var.get(), self.g_var.get(), self.b_var.get()
        threading.Thread(
            target=self.gs.send_color, args=(r, g, b), daemon=True,
        ).start()
